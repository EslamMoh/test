class BillingsController < ApplicationController
  ...
  def index
    ...
    # @first_value = params[:user_id]
    # //session[passed_variable] = @first_value
    if current_user.admin?
      @billings = Billing.all
    else
      @billings = current_user.billings #currentuser is already declared using devise
    end
  ...
  private
    ...
    def billing_params
      params.require(:billing).permit(:amount, :token, :reciep_id)
    end
end
